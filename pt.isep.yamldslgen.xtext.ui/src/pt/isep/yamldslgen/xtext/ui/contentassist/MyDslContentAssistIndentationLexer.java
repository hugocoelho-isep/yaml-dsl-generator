package pt.isep.yamldslgen.xtext.ui.contentassist;
  ...
  import pt.isep.yamldslgen.xtext.ide.contentassist.antlr.internal.InternalMyDslLexer;
   ...
   * script contains.
   */
  public class MyDslContentAssistIndentationLexer extends InternalMyDslLexer {

      /**
       * Content-assist mode flag. In content assist the "EOF" fed to the lexer is
       * the cursor position, not the real end of a balanced document. If we flushed
       * the still-open blocks at EOF (as the runtime lexer does), the content-assist
       * parser would think the cursor is back at the top level and would only ever
       * propose the top-level workflow keys. Leaving the open blocks UNclosed is
       * exactly what tells the parser the cursor is nested inside them, so it
       * proposes the right members (triggers under {@code on:}, fields under a step,
       * etc.). Real dedents that occur before the cursor are still emitted by
       * {@link #handleIndentChange(int)} -- only the trailing EOF flush is suppressed.
       * Flip to {@code true} only to reproduce the runtime lexer's balanced output.
       */
      private static final boolean EMIT_TRAILING_END_BLOCKS_AT_EOF = false;
      ...
      public MyDslContentAssistIndentationLexer() {
          super();
          indentStack.push(0);
      }

      public MyDslContentAssistIndentationLexer(CharStream input) {
          super(input);
          indentStack.push(0);
      }

      public MyDslContentAssistIndentationLexer(CharStream input, RecognizerSharedState state) {
          super(input, state);
          indentStack.push(0);
      }
      ...
      public Token nextToken() {
          ...
          //     ignored for indentation â€” otherwise a deep-indented trailing
          ...
          // (4) EOF: in content-assist mode the EOF is the cursor, so we DO NOT
          //     flush the still-open blocks (see EMIT_TRAILING_END_BLOCKS_AT_EOF).
          //     Leaving them open is what makes proposals context-sensitive. Real
          //     dedents before the cursor were already emitted by handleIndentChange.
          if (t.getType() == Token.EOF && !eofProcessed) {
              eofProcessed = true;
              if (EMIT_TRAILING_END_BLOCKS_AT_EOF) {
       ...
       * followed by a more-indented block â€” i.e. the key is block-valued
       ...
       * key AND is neither a sequence item ('- ') nor a mapping entry ('key:') â€” i.e.
